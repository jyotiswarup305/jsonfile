...
async function updateComponentAttribute(components, componentName, attributeName) {
  ...
  // If the attribute doesn't exist, create it with type 'custom' and config
  if (!component.schema?.[cleanAttribute]) {
    ...
    updatedComponent.schema[cleanAttribute] = { 
      type: 'custom',
      field_type: 'Acquia-Connector-trimmed',
      options: []
    };
  ...
  // Skip if already 'custom'
  if (attribute.type === 'custom') {
    return { updated: false, created: false, reason: 'existing_custom' };
  }

  // Skip if type is not 'text'
  if (attribute.type !== 'text') {
    return { updated: false, created: false, reason: 'wrong_type' };
  }

  // Update to 'custom' and add config
  const updatedComponent = JSON.parse(JSON.stringify(component));
  updatedComponent.schema[cleanAttribute].type = 'custom';
  updatedComponent.schema[cleanAttribute].field_type = 'Acquia-Connector-trimmed';
  updatedComponent.schema[cleanAttribute].options = [];
...
async function processUpdates() {
  ...
  for (const [index, row] of attributeUpdates.entries()) {
    ...
    if (result.updated) {
      console.log(`✓ Updated: Component '${compName}'—attribute '${attrName}' set to custom.`);
      results.updated++;
    } else if (result.created) {
      console.log(`✓ Created: Attribute '${attrName}' for component '${compName}' with type custom.`);
      results.created++;
    } else {
