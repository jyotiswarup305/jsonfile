...
async function updateComponentAttribute(components, componentName, attributeName) {
  ...
  // If the attribute doesn't exist, we'll create it with type 'text'
  ...
  // Update attribute from textarea to text.
  const updatedComponent = JSON.parse(JSON.stringify(component));
  updatedComponent.schema[cleanAttribute].type = 'text';
...
async function processUpdates() {
  ...
  for (const [index, row] of attributeUpdates.entries()) {
    ...
    if (result.updated) {
      console.log(`✓ Updated: Component '${compName}'—attribute '${attrName}' set to text.`);
      results.updated++;
    } else if (result.created) {
      console.log(`✓ Created: Attribute '${attrName}' for component '${compName}' with type text.`);
      results.created++;
    } else {
